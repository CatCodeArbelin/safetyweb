...
ЛадНет — Telegram-бот для оформления индивидуальных подписок на цифровой сервис защищённого сетевого доступа через внешний 3x-ui. Бот показывает тарифы, создаёт MVP-заявку на оплату, уведомляет администраторов, после ручного подтверждения оплаты создаёт клиента в 3x-ui и отправляет пользователю ссылку для защищённого соединения.
...
# Free test mode: users receive protected access links without payment/admin confirmation.
TEST_MODE=false
```
...
- Публичные тексты используют нейтральную формулировку `цифровой сервис защищённого сетевого доступа`.
...
- База данных хранит пользователей, подписки, платежи, ноды защищённого доступа и одноразовые события уведомлений о сроке подписки.
